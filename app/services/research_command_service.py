...
class ResearchCommandService:
    ...
    def _help_text() -> str:
        return (
            "调研命令：\n"
            "1) 调研 主题：{topic} 年份：2021-2026 领域：xxx 数量：20 来源：semantic_scholar|openalex|arxiv|dblp\n"
            "2) 调研 状态\n"
            "3) 调研 图谱 查看 [方向 k]\n"
    ...
    def _parse_topic_payload(payload: str) -> tuple[str, dict] | str:
        ...
        if sources_value:
            ...
            allowed = {"semantic_scholar", "openalex", "arxiv", "dblp"}
            filtered = [src for src in sources if src in allowed]
            if not filtered:
                return "来源仅支持 semantic_scholar、openalex、arxiv 或 dblp（可用 | 分隔）。"
