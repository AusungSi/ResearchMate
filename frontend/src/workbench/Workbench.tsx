...
export function Workbench() {
  ...
  const collectionDetailQuery = useQuery({
    queryKey: ["collection", activeCollectionId],
    queryFn: () => apiFetch<CollectionSummary>(`/api/v1/research/collections/${activeCollectionId}`),
  ...
  const graphQuery = useQuery({
    ...
    queryFn: () => apiFetch<GraphResponse>(`/api/v1/research/tasks/${activeTaskId}/graph?view=tree&include_papers=true&paper_limit=16`),
    enabled: Boolean(activeTaskId),
    refetchInterval: 5000,
  });
  ...
  const buildCollectionGraph = useMutation({
    mutationFn: async (collectionId: string) =>
      apiFetch(`/api/v1/research/collections/${collectionId}/graph/build`, { method: "POST" }),
    onSuccess: () => {
  ...
  const importZotero = useMutation({
    mutationFn: async () =>
      apiFetch<ZoteroImportResponse>("/api/v1/research/integrations/zotero/import", {
        method: "POST",
        body: JSON.stringify({ project_id: activeProjectId, collection_name: `Zotero 导入 ${new Date().toLocaleTimeString()}` }),
      }),
    onSuccess: (data) => {
      setActiveCollectionId(data.collection.collection_id);
      setActionStatus({ tone: "success", text: `已导入 Zotero：${data.collection.name}` });
      client.invalidateQueries({ queryKey: ["collections", activeProjectId] });
    },
    onError: (cause) => {
      setActionStatus({ tone: "danger", text: cause instanceof Error ? cause.message : String(cause) });
    ...
    <AppShell
      ...
      sidebar={
        <ProjectSidebar
          ...
          onImportZotero={() => importZotero.mutate()}
        />
      }
      canvas={
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-1 text-xs text-slate-400">
                  系统节点来自 canonical graph，手工节点与手工连线只写入 canvas state。多选论文卡片后可以直接加入 Collection。
                </div>
              </div>
      ...
      detail={
        <aside className="h-full overflow-auto bg-slate-50/60 p-5">
          ...
          {selectedNode && isPaperNode(selectedNode.id) ? <PdfPanel pdfUrl={pdfUrl} assets={paperAssetQuery.data || null} onClose={() => setPdfUrl("")} /> : null}
        </aside>
      }
    />
  );
...
function buildSuccessText(action: WorkbenchAction) {
  switch (action.type) {
    ...
    case "next_round":
      return `已根据新意图创建第 ${action.roundId} 轮后的下一轮探索。`;
    case "save_paper":
      return "论文已保存。";
    case "summarize_paper":
      return "论文总结已提交。";
